...
namespace Packet {
    ...
    struct A {                                // Packet A sent by the game sends the interesting information
            ...
            float fuelCapacity; // Max fuel capacity for current car (RANGE: 100 (most cars) -> 5 (karts) -> 0 (electric
                                // cars))
            ...
            int16_t
                RaceStartPosition;  // Position of the car before the start of the race, defaults to -1 after race start
            ...
            uint8_t gears;          // First 4 bits: Current Gear, Last 4 bits: Suggested Gear, see appendices
                                    // (getCurrentGearFromByte and getSuggestedGearFromByte)
            ...
            float roadPlaneDistance; // Distance above or below the plane, e.g a dip in the road is negative, hill is
                                     // positive.
    ...
    A parse( const std::array<uint8_t, 296>& bytes );

    static_assert( sizeof( A ) == 296 ); // padding check of packet a
} // namespace Packet
